...
namespace xfdtd {
OldTFSF::OldTFSF(SpatialIndex distance_x, SpatialIndex distance_y,
...
void OldTFSF::init(const Cube *simulation_box, double dx, double dy, double dz,
  ...
  initOldTFSF(simulation_box, dx, dy, dz, dt, tfsf_boundary_index);
  allocateKDotR();
  allocateEiHi();
  calculateKDotR();
}

void OldTFSF::initOldTFSF(const Cube *simulation_box, double dx, double dy, double dz,
                    double dt, OldTFSFBoundaryIndex tfsf_boundary_index) {
  if (simulation_box == nullptr) {
    throw std::runtime_error("OldTFSF::initOldTFSF() simulation_box is nullptr");
  }
