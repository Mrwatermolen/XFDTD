...
void TFSF2D::init(double dx, double dy, double dz, double dt,
  ...
  for (auto i{li}; i < ri + 1; ++i) {
    auto vec_ez_yn{PointVector{static_cast<double>(i - getStrikeIndexX()),
                               static_cast<double>(lj - getStrikeIndexY()), 0}};
    // auto k_dot_ez_yn{k_inc.transpose() * vec_ez_yn};
    auto k_dot_ez_yn{xt::eval(xt::linalg::dot(k_inc, vec_ez_yn))()};
    _projection_index_ez_yn[i - li] =
        std::floor(k_dot_ez_yn) + 1;  // 2 for boundary
    _weight_ez_yn[i - li] = k_dot_ez_yn - _projection_index_ez_yn[i - li] + 1;

    auto vec_hx_yn{PointVector{static_cast<double>(i - getStrikeIndexX()),
                               lj - 0.5 - getStrikeIndexY(), 0}};
    // auto k_dot_hx_yn{k_inc * vec_hx_yn};
    auto k_dot_hx_yn{xt::eval(xt::linalg::dot(k_inc, vec_hx_yn))()};
    _projection_index_hx_yn[i - li] = std::floor(k_dot_hx_yn) + 1;
    _weight_hx_yn[i - li] = k_dot_hx_yn - _projection_index_hx_yn[i - li] + 1;

    auto vec_ez_yp{PointVector{static_cast<double>(i - getStrikeIndexX()),
                               static_cast<double>(rj - getStrikeIndexY()), 0}};
    // auto k_dot_ez_yp{k_inc * vec_ez_yp};
    auto k_dot_ez_yp{xt::eval(xt::linalg::dot(k_inc, vec_ez_yp))()};
    _projection_index_ez_yp[i - li] = std::floor(k_dot_ez_yp) + 1;
    _weight_ez_yp[i - li] = k_dot_ez_yp - _projection_index_ez_yp[i - li] + 1;

    auto vec_hx_yp{PointVector{static_cast<double>(i - getStrikeIndexX()),
                               rj + 0.5 - getStrikeIndexY(), 0}};
    // auto k_dot_hx_yp{k_inc.transpose() * vec_hx_yp};
    auto k_dot_hx_yp{xt::eval(xt::linalg::dot(k_inc, vec_hx_yp))()};
    _projection_index_hx_yp[i - li] = std::floor(k_dot_hx_yp) + 1;
    _weight_hx_yp[i - li] = k_dot_hx_yp - _projection_index_hx_yp[i - li] + 1;
  }

  for (auto j{lj}; j < rj + 1; ++j) {
    auto vec_ez_xn{PointVector{static_cast<double>(li - getStrikeIndexX()),
                               static_cast<double>(j - getStrikeIndexY()), 0}};
    // auto k_dot_ez_xn{k_inc.transpose() * vec_ez_xn};
    auto k_dot_ez_xn{xt::eval(xt::linalg::dot(k_inc, vec_ez_xn))()};
    _projection_index_ez_xn[j - lj] = std::floor(k_dot_ez_xn) + 1;
    _weight_ez_xn[j - lj] = k_dot_ez_xn - _projection_index_ez_xn[j - lj] + 1;

    auto vec_hy_xn{PointVector{li - 0.5 - getStrikeIndexX(),
                               static_cast<double>(j - getStrikeIndexY()), 0}};
    // auto k_dot_hy_xn{k_inc.transpose() * vec_hy_xn};
    auto k_dot_hy_xn{xt::eval(xt::linalg::dot(k_inc, vec_hy_xn))()};
    _projection_index_hy_xn[j - lj] = std::floor(k_dot_hy_xn) + 1;
    _weight_hy_xn[j - lj] = k_dot_hy_xn - _projection_index_hy_xn[j - lj] + 1;

    auto vec_ez_xp{PointVector{static_cast<double>(ri - getStrikeIndexX()),
                               static_cast<double>(j - getStrikeIndexY()), 0}};
    // auto k_dot_ez_xp{k_inc.transpose() * vec_ez_xp};
    auto k_dot_ez_xp{xt::eval(xt::linalg::dot(k_inc, vec_ez_xp))()};
    _projection_index_ez_xp[j - lj] = std::floor(k_dot_ez_xp) + 1;
    _weight_ez_xp[j - lj] = k_dot_ez_xp - _projection_index_ez_xp[j - lj] + 1;

    auto vec_hy_xp{PointVector{ri + 0.5 - getStrikeIndexX(),
                               static_cast<double>(j - getStrikeIndexY()), 0}};
    // auto k_dot_hy_xp{k_inc.transpose() * vec_hy_xp};
    auto k_dot_hy_xp{xt::eval(xt::linalg::dot(k_inc, vec_hy_xp))()};
    _projection_index_hy_xp[j - lj] = std::floor(k_dot_hy_xp) + 1;
    _weight_hy_xp[j - lj] = k_dot_hy_xp - _projection_index_hy_xp[j - lj] + 1;
  }
}
...
void TFSF2D::updateIncidentField(size_t current_time_step) {
  ...
  if (current_time_step >= 1800) {
    std::fstream f(
        "visualizing_data/" + std::to_string(current_time_step) + ".dat",
        std::ios::out);
    for (auto &&e : _e_inc) {
      f << e << " ";
    }
